...
public class GasChannelSettings extends DefaultChannelSettings implements IChannelSettings {
	...
	private void tickGasHandler(IControllerContext context, GasConnectorSettings settings, BlockPos extractorPos, IGasHandler handler) {
		...
		GasStack stack = fetchGas(handler, true, settings.getFilter(), settings.getFacing(), rate);
		if (stack == null) {
			return;
		}

		// This matches XNet's overall flow more closely:
		// spend RF once per extraction attempt, then try to move gas.
	...
	@Nullable
	private GasStack fetchGas(IGasHandler handler, boolean simulate, @Nullable GasStack filter, EnumFacing side, int extractAmount) {
		if (extractAmount <= 0) {
			return null;
		}
		...
		if (filter != null && !filter.isGasEqual(stack)) {
			return null;
		}
	...
	public boolean transferGas(@Nonnull IGasHandler from,
		...
		for (int j = 0; j < gasConsumers.size(); j++) {
			...
			if (!matchesFilter(insertSettings, stack)) {
				continue;
			}
			...
			// Match XNet-style round robin behavior:
			// first successful destination wins this operation
	...
	public int insertGasToHandler(@Nonnull IGasHandler from,
		...
		if (filled > 0) {
			GasStack realExtract = fetchGas(from, false, extractSettings.getFilter(), extractSettings.getFacing(), filled);
	...
	private boolean matchesFilter(GasConnectorSettings settings, GasStack stack) {
		GasStack filter = settings.getFilter();
		return filter == null || filter.isGasEqual(stack);
	}
	...
	public void createGui(IEditorGui gui) {
		gui.nl().choices(TAG_MODE, "Gas channel mode", channelMode, ChannelMode.values());
	}
