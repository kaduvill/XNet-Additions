...
public final class RemoteGuiConnector extends GuiConnector implements RemoteConnectorNetwork.Receiver {
    ...
    public RemoteGuiConnector(GuiController controllerGui, RemoteConnectorNetwork.Response response) {
        ...
        this.controllerPos = response.getControllerPos();
    ...
    private static ConnectorTileEntity snapshot(RemoteConnectorNetwork.Response response) {
        ...
        connector.setPos(response.getTarget().getPos().offset(response.getTarget().getSide()));
        connector.setConnectorName(response.getName());
        for (EnumFacing facing : EnumFacing.VALUES) {
            connector.setEnabled(facing, (response.getEnabledMask() & 1 << facing.ordinal()) != 0);
        }
        return connector;
    }
    ...
    public void sendServerCommand(SimpleNetworkWrapper network, String command, TypedMap params) {
        if (GenericTileEntity.COMMAND_SYNC_BINDING.equals(command) && params.size() == 1) {
            String name = params.get(ConnectorTileEntity.VALUE_NAME);
            if (name == null) {return;}
            tileEntity.setConnectorName(name);
            RemoteConnectorNetwork.CHANNEL.sendToServer(RemoteConnectorNetwork.Request.name(controllerPos, target, requestId, name));
        } else if (ConnectorTileEntity.CMD_ENABLE.equals(command) && params.size() == 2) {
            Integer facing = params.get(ConnectorTileEntity.PARAM_FACING);
            Boolean enabled = params.get(ConnectorTileEntity.PARAM_ENABLED);
            if (facing == null || facing < 0 || facing >= EnumFacing.VALUES.length || enabled == null) {return;}
            tileEntity.setEnabled(EnumFacing.VALUES[facing], enabled);
            RemoteConnectorNetwork.CHANNEL.sendToServer(RemoteConnectorNetwork.Request.side(controllerPos, target, requestId, facing, enabled));
        }
    }
    ...
    @Override
    public void xnetadditions$receiveRemoteConnector(RemoteConnectorNetwork.Response response) {
        if (response.getRequestId() != requestId || !controllerPos.equals(response.getControllerPos())
                || !target.equals(response.getTarget()) || response.getKind() == RemoteConnectorNetwork.OPEN) {return;}
        if (!response.getMessage().isEmpty()) {Logging.warn(mc.player, response.getMessage());}
